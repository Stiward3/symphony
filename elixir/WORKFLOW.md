...
- This is retry attempt #{{ attempt }} because the Jira issue is still in an active state.
...
## Prerequisite: Jira access is configured

Symphony is polling Jira for this project. Use the normalized `issue` fields provided in the prompt as the source of truth.

When you need to update Jira from inside the session:
- use `jira_issue_update` to add or refresh the workpad comment
- use `jira_issue_update` to move the issue to the correct Jira state
- once the requested deliverable exists and the completion bar is satisfied, immediately use `jira_issue_update` to move the issue out of `In Progress` and stop; do not spend extra turns polishing or re-evaluating already-finished work

If Jira updates or comments require tooling that is unavailable in-session, stop and report the blocker clearly.
...
- When meaningful out-of-scope improvements are discovered during execution,
  ...
  must include a clear title, description, and acceptance criteria and stay in
  the same Jira project as the current issue.
- Move status only when the matching quality bar is met.
- Operate autonomously end-to-end unless blocked by missing requirements, secrets, or permissions.
- Use the blocked-access escape hatch only for true external blockers (missing required tools/auth) after exhausting documented fallbacks.
- For small, concrete deliverables such as a mock JSON fixture, script, config sample, or focused bug fix, prefer one decisive implementation pass over long reasoning loops.
- If the requested artifact has already been created and validated in the workspace, update Jira immediately and end the turn instead of continuing analysis.
...
1. If substantial review feedback or design changes invalidate the current approach, treat the next attempt as a fresh execution cycle.
2. Re-read the full issue body and all comments; explicitly identify what will be done differently this attempt.
