defmodule SymphonyElixir.ExtensionsTest do
  ...
  test "tracker delegates to memory and linear adapters" do
    ...
    assert {:ok, ["In Progress"]} = SymphonyElixir.Tracker.fetch_issues_by_states(["In Progress"])
    assert_receive {:jira_fetch_issues_by_states_called, ["In Progress"]}
    assert {:ok, ["PROJ-1"]} = SymphonyElixir.Tracker.fetch_issue_states_by_ids(["PROJ-1"])
    assert_receive {:jira_fetch_issue_states_by_ids_called, ["PROJ-1"]}
    assert :ok = SymphonyElixir.Tracker.create_comment("PROJ-1", "jira comment")
    assert_receive {:jira_create_comment_called, "PROJ-1", "jira comment"}
    assert :ok = SymphonyElixir.Tracker.update_issue_state("PROJ-1", "Done")
    assert_receive {:jira_update_issue_state_called, "PROJ-1", "Done"}
  end
